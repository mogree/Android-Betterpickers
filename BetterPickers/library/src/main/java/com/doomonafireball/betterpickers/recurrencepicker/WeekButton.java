...
public class WeekButton extends android.widget.ToggleButton {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (h > 0 && w > 0) {
            if (w < h) {
                if (MeasureSpec.getMode(ViewCompat.getMeasuredHeightAndState(this)) != MeasureSpec.EXACTLY) {
                    h = w;
                }
            } else if (h < w) {
                if (MeasureSpec.getMode(ViewCompat.getMeasuredWidthAndState(this)) != MeasureSpec.EXACTLY) {
                    w = h;
                }
            }
        }
